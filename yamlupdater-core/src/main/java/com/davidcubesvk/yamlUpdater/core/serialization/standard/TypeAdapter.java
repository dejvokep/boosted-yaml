...
public interface TypeAdapter<T> {
     ...
     * the top-level map (the one returned), where the key is defined by the serializer (<code>==</code> for
     * {@link StandardSerializer#DEFAULT}) and the value identifies the serialized type - either by the full classname
     * (e.g. <code>me.name.project.objects.CustomObject</code>) or it's alias. <b>Both must also be registered</b>.
     * <p>
